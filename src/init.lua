local lzss, component, computer = ..., component, computer
--_FLASH = {}
...
xpcall(function()
	...
	bt = bt and bt:byte()
	if bt == 1 then
		frboot(config[1--[[BOOT_ADDRESS]]], string.unpack("H", config[11--[[BOOT_PORT]]]), config[2--[[BOOT_PATH]]])
	elseif bt == 0 or not bt then
		local addr = b2a(config[1--[[BOOT_ADDRESS]]])
		local ct = component.type(addr)
