...
local function prompt(query, dev)
	...
	local i = input(ox+2, oy+1, pwidth, "[%w ]", dev)
	local val
...
local function insert_devs(lab, ct)
	...
	for i=1, #dl do
		local e = {
			l = string.format("%s: %s (%s)", lab, (ct == "modem") and dl[i]:sub(1,3) or component.invoke(dl[i], "getLabel"), dl[i]:sub(1, 3)),
			v = dl[i]
		}
		table.insert(devs, e)
		devs[e.v] = e.l
...
local labels = {
	...
	{"Boot path", 					"path", 		nil, add_item(button, "<select>", nofunc)},
...
local keys = {
	...
	{11, "port", function(b) return tostring(string.unpack("H", b)) end, function(b) return string.pack("H", tonumber(b)) end}
}
...
local function save_config()
	for i=1, #keys do
		local key = keys[i]
		ziptie.cfg.set(key[1], key[4](kv[key[2]]))
	end
end
